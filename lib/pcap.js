...
function tsharkJson(args) {
  return new Promise((resolve, reject) => {
    ...
    if (!tshark) return reject(new Error('TShark not found. Install Wireshark (including tshark CLI).'));
    ...
    child.on('close', code => {
      if (code !== 0 && !stdout) return reject(new Error(`tshark error (${code}): ${stderr || 'Unknown error'}`));
      try { resolve(JSON.parse(stdout)); } catch { resolve(stdout); }
    });
    child.on('error', err => reject(new Error(`Failed to spawn tshark: ${err.message}`)));
  });
}
...
async function getPackets(id, filter, limit = 200, offset = 0) {
  const { filePath } = await getPcapFile(id);
  const fields = ['frame.number', 'frame.time', 'ip.src', 'ip.dst', 'ip.proto', 'tcp.srcport', 'tcp.dstport', 'udp.srcport', 'udp.dstport', 'frame.len', '_ws.col.Protocol', 'tcp.flags.syn', 'tcp.flags.ack', 'tcp.flags.fin', 'tcp.flags.reset', 'http.request.method', 'http.response.code', 'dns.qry.name', 'tls.handshake.type'];
...
function getPacketInfo(pkt) {
  ...
  if (layers['http.request.method']) return `${layers['http.request.method'][0]} ${layers['http.request.uri']?.[0] || '/'}`;
  if (layers['http.response.code']) return `HTTP ${layers['http.response.code'][0]}`;
...
async function startLiveCapture(interfaceName, duration, filter) {
  if (activeCaptures[interfaceName]) throw new Error(`Capture already running on ${interfaceName}`);
  const tshark = findTshark();
  if (!tshark) throw new Error('TShark not found');
...
function validateDisplayFilter(filter) {
  return new Promise((resolve) => {
    ...
    const tshark = findTshark();
    if (!tshark) return resolve({ valid: false, message: 'TShark not available for filter validation' });
    const child = spawn(tshark, ['-Y', filter, '-r', path.join(PCAP_DIR, 'dummy')], { windowsHide: true });
    let stderr = '';
    child.stderr.on('data', d => stderr += d);
    child.on('close', code => {
      if (code !== 0 && (stderr.includes('Invalid') || stderr.includes('Error'))) resolve({ valid: false, message: stderr.trim() });
...
module.exports = {
  ...
  getConversations, getPackets, getCaptureMetadata, listInterfaces,
  startLiveCapture, stopLiveCapture, getActiveCaptures,
